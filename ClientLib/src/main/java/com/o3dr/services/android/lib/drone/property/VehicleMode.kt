...
enum class VehicleMode(val mode: Int, val droneType: Int, val label: String) : DroneAttribute {
    ...
    COPTER_FLOW_HOLD(22, Type.TYPE_COPTER, "Flow Hold"),
    ...
    ROVER_SMART_RTL(12, Type.TYPE_ROVER, "Smart RTL"),
    ...
    companion object {
        @JvmField
        val CREATOR: Parcelable.Creator<VehicleMode> = object : Parcelable.Creator<VehicleMode> {
            override fun createFromParcel(source: Parcel): VehicleMode? {
                return valueOf(source.readString())
            }

            override fun newArray(size: Int): Array<VehicleMode?> {
                return arrayOfNulls(size)
            }
        }

        @JvmStatic
        fun getVehicleModePerDroneType(droneType: Int): List<VehicleMode> {
            val availableModes = values()
            val vehicleModes: MutableList<VehicleMode> = ArrayList(availableModes.size)
            for (vehicleMode in availableModes) {
                if (vehicleMode.droneType == droneType) vehicleModes.add(vehicleMode)
            }
            return vehicleModes
        }
    }
