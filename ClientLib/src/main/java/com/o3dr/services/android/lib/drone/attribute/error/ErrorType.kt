 ...
 */
enum class ErrorType(@field:StringRes @param:StringRes private val labelResId: Int) : Parcelable {
    ...
    fun getLabel(context: Context?): CharSequence? {
        return context?.getText(labelResId)
    }
    ...
    companion object {
        ...
        @JvmField
        val CREATOR: Parcelable.Creator<ErrorType?> = object : Parcelable.Creator<ErrorType?> {
            override fun createFromParcel(source: Parcel): ErrorType? {
                return valueOf(source.readString())
            }

            override fun newArray(size: Int): Array<ErrorType?> {
