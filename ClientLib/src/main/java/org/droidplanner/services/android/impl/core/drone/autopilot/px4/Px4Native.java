...
import org.droidplanner.services.android.impl.core.MAVLink.PX4WaypointManager;
...
public class Px4Native extends GenericMavLinkDrone {
    private static final String TAG = Px4Native.class.getSimpleName();
    ...
    public Px4Native(String droneId, Context context, Handler handler, DataLink.DataLinkProvider<MAVLinkMessage> mavClient, AutopilotWarningParser warningParser, LogMessageListener logListener) {
        ...
        this.waypointManager = new PX4WaypointManager(this, handler);
        this.rc = new RC(this);
...
//        }
    ...
    protected void processAltitude(msg_altitude msg) {
        super.processAltitude(msg);

        final double alt = altitude.getAltitude();
        if(Double.isNaN(alt) || alt == 0) {
            altitude.setAltitude(msg.altitude_relative);
        }
    }
...
//            final Px4Mode mode = Px4Mode.getHeartbeatMode(msg.base_mode, customMode.sub_mode, msg.type);
    ...
    @Override
    protected boolean setVehicleMode(final Bundle data, final ICommandListener listener) {
        ...
        Timber.d("setVehicleMode(): newMode=%s", newMode);
    ...
    public boolean executeAsyncAction(Action action, final ICommandListener listener) {
        ...
        Timber.d("executeAsyncAction(%s)", type);
...
//                    CommonApiUtils.postErrorEvent(CommandExecutionError.COMMAND_FAILED, listener);
    ...
    // TODO: TOTALLY WRONG for Px4
    protected void processStatusText(msg_statustext statusText) {
        Log.v(TAG, String.format("processStatusText(): %s sysid=%d compid=%d", 
            statusText.getText(), statusText.sysid, statusText.compid));
        ...
        if (message.startsWith("ArduCopter") || message.startsWith("ArduPlane")
            ...
            if (!getState().parseAutopilotError(message)) {
                ...
                if(message.toLowerCase().startsWith("preflight fail:")) {
                    logLevel = Log.ERROR;
                }
...
//                Px4Modes vehicleMode = (Px4Modes)getState().getMode().getNativeMode();
//                if (Px4Modes.isCopter(vehicleMode.getType())) {
